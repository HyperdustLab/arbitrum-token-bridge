...
import { ArrowLeftOnRectangleIcon, ArrowTopRightOnSquareIcon, ChevronDownIcon, Cog6ToothIcon, DocumentDuplicateIcon, DocumentTextIcon } from '@heroicons/react/24/outline'
...
export function HeaderAccountPopover({
  ...
  const { address, accountShort, ensName, ensAvatar, openTransactionHistory, disconnect, udInfo, chain, setQueryParams } = useAccountMenu()
  ...
  const headerItemsClassName = 'arb-hover flex w-full flex-row items-center space-x-2 px-12 py-2 text-sm sm:text-sm text-gray-4 sm:text-white hover:bg-ocl-blue sm:px-4 sm:py-1'
    ...
    <Popover className="relative w-full px-4 sm:w-max sm:p-0">
      <Popover.Button
        className={twMerge(
          ...
          isTestnet ? 'sm:border-white sm:ui-not-open:bg-white/20' : 'sm:border-gray-1 sm:ui-not-open:bg-gray-1 sm:ui-not-open:hover:bg-white/10'
        )}
        style={destinationChainStyle}
        role="button"
        aria-label="Account Header Button"
      >
        <SafeImage src={ensAvatar || undefined} className="h-6 w-6 rounded-full sm:h-8 sm:w-8" fallback={<CustomBoringAvatar size={isSmallScreen ? 24 : 40} name={address} />} />
        <span className="flex flex-col text-justify text-base leading-extra-tight text-gray-4 sm:text-white">
          ...
          {isSmartContractWallet && !isLoadingAccountType && <span className="text-[10px]">Smart Contract Wallet</span>}
        </span>
      ...
      <Transition className="w-full sm:absolute sm:top-0">
        <Popover.Panel className="flex w-full flex-col overflow-hidden rounded pb-2 sm:absolute sm:top-0 sm:bg-dark">
          ...
          {showCopied && <span className="absolute right-2 top-4 z-50 text-xs text-white">Copied to clipboard!</span>}
          <div className="relative hidden w-full flex-row items-center px-2 pb-2 pt-3 sm:flex">
            ...
            <div className="absolute inset-0 flex h-8 w-full flex-col items-center overflow-hidden bg-dark text-center">
              <div className="scale-400 blur-2xl filter">
                <SafeImage className="h-100 w-100 rounded-full" src={ensAvatar || undefined} fallback={<CustomBoringAvatar size={200} name={address} />} />
              </div>
            </div>
            ...
            <div className="relative flex flex-row items-center gap-2">
              <div className="avatar-container box-content rounded-full border-[3px] border-dark">
                <SafeImage src={ensAvatar || undefined} className="h-[54px] w-[54px] rounded-full" fallback={<CustomBoringAvatar size={54} name={address} />} />
              </div>
              <button className="flex translate-y-1 flex-row items-center gap-1 text-white/70 hover:text-white" onClick={() => copy(ensName ?? udInfo.name ?? address ?? '')}>
                <span className="max-w-[10rem] overflow-hidden text-ellipsis text-sm">{ensName ?? udInfo.name ?? accountShort}</span>
          ...
          <div className="flex w-full flex-col justify-between sm:flex-col sm:items-end sm:px-0">
            ...
            {isCorrectNetworkConnected && (
              <button className={headerItemsClassName} onClick={openTransactionHistory}>
            ...
            {isCorrectNetworkConnected && chain && (
              <ExternalLink href={`${getExplorerUrl(chain.id)}/address/${address}`} className={headerItemsClassName}>
            ...
            {/* Settings */}
            {isCorrectNetworkConnected && (
              <button className={headerItemsClassName} onClick={() => setQueryParams({ settingsOpen: true })}>
